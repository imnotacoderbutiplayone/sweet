...
with tabs[3]:
    ...
    try:
        ...
        qf_left = parse_json_field(bracket_data.get("qf_left", "[]"))
        qf_right = parse_json_field(bracket_data.get("qf_right", "[]"))
        sf_left = parse_json_field(bracket_data.get("sf_left", "[]"))
        sf_right = parse_json_field(bracket_data.get("sf_right", "[]"))
    except Exception as e:
        st.error(f"❌ Failed to load bracket progression: {e}")
        r16_left = r16_right = qf_left = qf_right = sf_left = sf_right = []
        bracket_data = {}
        field_locked = False
    ...
    if st.session_state.authenticated:
        ...
        with col1:
            ...
            for i, (p1_name, p2_name) in enumerate(r16_left):
                ...
                default_winner = safe_get(qf_left, i // 2)
            ...
            for i in range(0, len(r16_left_results), 2):
                if i + 1 < len(r16_left_results):
                    ...
                    default_winner = safe_get(sf_left, i // 2)
                    winner_name = render_match(p1, p2, default_winner, readonly=False, key_prefix=f"qf_left_{i}")
                    qf_left_results.append(get_winner_player(p1, p2, winner_name))
        ...
        with col2:
            ...
            for i, (p1_name, p2_name) in enumerate(r16_right):
                p1 = get_player_by_name(p1_name, bracket_df)
                p2 = get_player_by_name(p2_name, bracket_df)
                default_winner = safe_get(qf_right, i // 2)
            ...
            for i in range(0, len(r16_right_results), 2):
                if i + 1 < len(r16_right_results):
                    ...
                    default_winner = safe_get(sf_right, i // 2)
                    winner_name = render_match(p1, p2, default_winner, readonly=False, key_prefix=f"qf_right_{i}")
                    qf_right_results.append(get_winner_player(p1, p2, winner_name))

        # --- Semifinals & Final ---
        st.markdown("### 🏁 Semifinals & Final")
        sf_left_finalists = qf_left_results
        sf_right_finalists = qf_right_results

        if sf_left_finalists and sf_right_finalists:
            default_finalist_left = bracket_data.get("finalist_left")
            default_finalist_right = bracket_data.get("finalist_right")
            finalist_left = render_match(sf_left_finalists[0], sf_left_finalists[1], default_finalist_left, readonly=False, key_prefix="sf_left")
            finalist_right = render_match(sf_right_finalists[0], sf_right_finalists[1], default_finalist_right, readonly=False, key_prefix="sf_right")
            finalist_left_player = get_winner_player(sf_left_finalists[0], sf_left_finalists[1], finalist_left)
            finalist_right_player = get_winner_player(sf_right_finalists[0], sf_right_finalists[1], finalist_right)
        ...
        if st.button("📋 Save Bracket Progress"):
            try:
                updates = {}
                if qf_left_results: updates["qf_left"] = [p["name"] for p in qf_left_results]
                if qf_right_results: updates["qf_right"] = [p["name"] for p in qf_right_results]
                if sf_left_finalists: updates["sf_left"] = [p["name"] for p in sf_left_finalists]
                if sf_right_finalists: updates["sf_right"] = [p["name"] for p in sf_right_finalists]
    ...
    # --- Read-Only View ---
        ...
        with col1:
            ...
            render_matchups("#### 🥉 Quarterfinals", qf_left)
            render_matchups("#### 🥈 Semifinalist", sf_left)

        with col2:
            st.markdown("### 🟥 Right Side")
            render_matchups("#### 🔹 Round of 16", r16_right)
            render_matchups("#### 🥉 Quarterfinals", qf_right)
            render_matchups("#### 🥈 Semifinalist", sf_right)
