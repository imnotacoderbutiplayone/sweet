...
if "bracket_data" not in st.session_state:
    if os.path.exists(BRACKET_FILE):
        with open(BRACKET_FILE, "r") as f:
            st.session_state.bracket_data = pd.read_json(f, orient="split")
    else:
...
if st.session_state.authenticated:
    ...
    if st.sidebar.button("Clear all Match Data"):
        ...
        save_json(RESULTS_FILE, {})  # Clear saved match results
        with open(BRACKET_FILE, "w") as f:
            f.write("{}")  # Clear saved bracket data
...
with tabs[0]:
    ...
    # Function to style table with alternating row colors
    def style_table(df):
...
with tabs[1]:
    ...
    # Simulate match results for each pod
    ...
    if st.session_state.authenticated:
        st.sidebar.subheader("Admin Actions")
        if st.sidebar.button("Clear all Match Data"):
            # Reset session state and clear saved data
            st.session_state.match_results = {}
            st.session_state.bracket_data = pd.DataFrame()
            save_json(RESULTS_FILE, {})  # Clear saved match results
            with open(BRACKET_FILE, "w") as f:
                f.write("{}")  # Clear saved bracket data
            st.success("Match data has been cleared.")
...
with tabs[3]:
    ...
    if st.session_state.bracket_data.empty:
        ...
        left_side = bracket_df.iloc[:8].reset_index(drop=True)
        ...
        with col1:
            ...
            for i in range(0, 8, 2):
                p1, p2 = left_side.iloc[i], left_side.iloc[i + 1]
            ...
            finalist_left = st.radio("🏅 Left Finalist:", [label(sf_left[0]), label(sf_left[1])], key="LFinal")
        ...
        with col2:
            ...
            qf_right = []
            for i in range(0, 8, 2):
                p1, p2 = right_side.iloc[i], right_side.iloc[i + 1]
            ...
            st.markdown("#### 🥇 Semifinal Winner")
            finalist_right = st.radio("🏅 Right Finalist:", [label(sf_right[0]), label(sf_right[1])], key="RFinal")
            finalist_right = sf_right[0] if finalist_right == label(sf_right[0]) else sf_right[1]

        # Final match
        st.markdown("### 🏁 Final Match")
        champion = st.radio("🏆 Champion:", [label(finalist_left), label(finalist_right)], key="Champ")
...
with tabs[2]:
    ...
    if st.session_state.match_results:  # Only display if match results are available
        ...
        # Aggregate match results from each pod
        ...
        if standings.empty:
            ...
            # Sort standings by points and margin
...
with tabs[5]:
    ...
    if st.session_state.bracket_data.empty:
        ...
        if username:
            ...
            bracket_df = st.session_state.bracket_data
            left = bracket_df.iloc[:8].reset_index(drop=True)
